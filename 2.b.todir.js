...
function compile(node) {
    ...
    code = 'var _data_=_data_||{}\n' // 
        ...
        + code;
    /*var*/
    render = Function('_data_', code);
    for (var dir in dirs) {
        ! function() {
            var fn = dirs[dir];
            render[dir] = function(node, arg, ext, value) {
                var args = {
                    node: node,
                    arg: arg,
                    ext: ext,
        ...
        if (dir) {
            ...
            switch (dir.name) {
                ...
                case 'var':
                    ...
                    break;
                default:
                    code += indent() + '_$$["#dir"](_$$.node(#nid), "#arg", "#ext", #value)'
...
directive('attr', function(args) {
    if (args.node[args.arg] == args.value) return;
    args.node[args.arg] = args.value
        // :class :style
